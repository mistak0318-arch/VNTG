...
export async function summarizeHits(
  ...
  /*
   * **출력 상한 1200 → 3000** (2026-09-05).
   *
   * 벤티지: "AI 정리 부분도 손봐야 돼. 사각 박스 안에 글자가 잘려 보인다."
   *
   * 화면이 자르는 게 아니었다 — **글이 실제로 잘려서 왔다.** 한국어는 토큰을 많이 먹어서
   * 1200 토큰이면 800~1000자쯤이다. 여기는 최대 80건을 받아 **세 단**으로 정리하라고
   * 시키는 자리라 그 안에 안 끝난다. 상한에 걸리면 `summarize` 가 마지막 성한 문장까지만
   * 남기고(단어 중간에서 끊긴 꼬리는 숫자를 오독하게 한다) 잘렸다고 알려 주는데,
   * **자주 걸리면 알림이 아니라 그냥 기본 동작**이 된다.
   *
   * 3000 이면 이 프롬프트가 시키는 세 단을 넉넉히 담는다. 호출당 비용은 출력 토큰만큼만
   * 늘고, 이건 **누를 때만** 도는 자리다(자동으로 안 돈다).
   */
  const res = await summarize(prompt, 3000, "channel");
  return {
    text: res.text,
