 ...
 *
 * ## 09/16 부터 **종류마다 다르다** (벤티지: 마감 정리를 15:40 으로 → 종배 스캔과 안 겹치게 15:55)
 *
 * 파이프라인이 15:55 로 앞당겨졌다 — 애프터장(16:00~20:00)에서 쓸 신호를 만들려는 것이다.
    ...
    for (const code of codes) {
      ...
      for (const kind of kinds) {
        ...
        try {
           ...
           *
           * ⚠️ **단, 셋에만** (2026-09-18 밤 #11 — 과잉 교정을 되돌린다). 처음엔 모든 종류에 걸었더니 20:10
           * ledgerFinal 이 「실패 137」로 튀었다. `short`(공매도)·`loan`(대차)는 **그 종목에 공매도·대차가 없으면
           * 빈 것이 정상**이라, 빈 응답을 실패로 세면 그 종목들이 도장을 못 받아 매 회차 영영 다시 받는다.
           * 수급(flow)·외인지분율(fgnRatio)·프로그램(prog)은 종목마다 날마다 있어야 하는 값이라 빈 것이 곧 실패다.
           */
          if (got.length === 0 && (kind === "flow" || kind === "fgnRatio" || kind === "prog")) throw new Error(`빈 응답 (${kind})`);
