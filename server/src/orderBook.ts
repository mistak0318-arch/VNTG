...
export async function orderBook(client: KiwoomClient, code: string): Promise<OrderBook> {
  ...
  try {
     ...
     * KRX 호가가 통째로 비면 `_NX` 로 다시 받는다 — 실측: 프리장 ka10004 `000660_NX` 가
     * 살아 있는 10단계를 줬다 (매수1 1,696,000 · 총잔량 27,617).
     * venue 로 어느 시장 호가인지 밝힌다.
     *
     * ⚠️ **판정 기준은 시각이 아니라 「KRX 총잔량이 0인가」다** — 그래서 2026-09-14
     * 애프터마켓 개편에도 손댈 것이 없다 (2026-09-12 확인). 그날부터 16:00~20:00 에는
     * KRX 호가가 살아 있으므로 폴백이 저절로 안 돌고, 15:30~16:00 공백에는 양쪽 다
     * 비어서 KRX 빈 호가가 그대로 남는다(없는 걸 있다고 하지 않는다).
     * 예전 주석이 적어 둔 시간대(프리 08:00~08:50 · 애프터 15:30~20:00)는 9/13 까지의
     * 이야기라 지웠다 — **동작은 그대로다.**
     */
