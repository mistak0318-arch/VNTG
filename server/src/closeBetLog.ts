...
export function startCloseBetScheduler(client: KiwoomClient): void {
  ...
  const tick = async () => {
     ...
     *
     * ## 9/14 뒤에도 **여기는 15:35 그대로다** (2026-09-12, KRX 애프터마켓 개편)
     *
     * 마감 뒤 정리는 20:10 으로 옮겼지만 이건 **「종가배팅」** — 정규장이 끝나는 그 순간의
     * 지수·선물을 찍어 두는 기록이다(채점도 `15:35 시점 선물` 기준으로 쌓여 있다). 20:10 로
     * 옮기면 뜻이 바뀌고 9/14 를 경계로 과거 채점과 안 맞는다.
     *
     * 오히려 새 시간표에서 15:35 는 **더 깨끗하다** — 9/13 까지는 15:30 직후부터 NXT 애프터가
     * 이어 받았지만, 9/14 부터 15:30~16:00 은 KRX 만 쉰다(NXT 는 15:30 단일가·15:40 연속으로 돈다 — 2026-09-16 키움 시간표로 바로잡음). 그래도 KRX 가격은 그 사이 안 움직인다.
     *
     * 창의 끝(16:05)이 애프터마켓 개장(16:00)을 5분 넘지만, `recordAndScore` 안에 「오늘 줄이
     * 이미 있으면 안 적는다」가 있어 두 번 찍히지는 않는다. 다만 15:35~16:00 에 한 번도 못
     * 걸린 날(그 사이 서버가 꺼져 있던 날)은 16:00~16:05 에 처음 찍히고, 그 값은 애프터
     * 5분을 머금는다 — 관측이 쌓이면 끝을 16:00 으로 당긴다.
     */
