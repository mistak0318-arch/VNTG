...
export async function stockSummary(client: KiwoomClient, code: string): Promise<StockSummary> {
   ...
   */
  /*
   * **한 쪽에 오는 만큼 다 보낸다** (2026-08-31 — "5일 20일 60일 120일 선택할 수
   * 있게"). ka10059 한 쪽이 100일쯤이라 여기서 자르지 않으면 그만큼 온다.
   *
   * 기간을 화면에서 고르게 하려면 **자르는 자리가 화면이어야** 한다. 서버가 20일로
   * 잘라 보내면 60일을 보려고 할 때마다 조회가 새로 나가는데, 이 응답은 30초마다
   * 폴링되는 것이라 그러면 안 된다. 넉넉히 보내고 화면이 고른다 — 조회는 그대로다.
   *
