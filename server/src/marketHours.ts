 ...
 *   15:30~20:00  NXT 애프터      15:30~16:00  **KRX 만 쉰다** — NXT 는 돈다(아래)
 *   16:00~18:00  시간외단일가    16:00~20:00  **KRX 애프터마켓** (+ NXT 애프터)
 *                               시간외단일가 **폐지**
 *
 * ⚠️ **15:30~16:00 은 「아무 시장도 안 연다」가 아니다** (2026-09-16 — 벤티지가 키움 공지의 시간표 그림을 붙여 줌,
 * openapi.kiwoom.com Board0101View). 9/12 에 키움 문자만 보고 「공백」이라 적었던 것을 바로잡는다.
 *   - NXT 애프터마켓: 15:30~15:40 **시가 단일가**, 15:40~20:00 연속매매. NXT 종가매매 15:30~16:00(호가 접수 15:00~)
 *   - KRX 장후 시간외 **종가**매매 15:40~16:00(당일 종가로만, 주문 접수 15:30~) · 시간외 대량/바스켓 15:40~20:00
 *   - KRX 연속매매는 16:00 애프터마켓부터
 * 그래서 이 30분의 이름은 「공백」 그대로 두되 뜻은 **「KRX 연속매매가 쉰다」**다 — KRX 가격은 정규장 종가에서 안 움직이고
 * (종가매매는 그 값으로만), NXT 만 움직인다. 주문 창(`orders.WINDOWS`)은 처음부터 NXT 15:30~20:00 이라 맞았다.
 *
 * **그 30분은 「거래 중」으로 치지 않는다 — 벤티지 결정(2026-09-16, 선택지)**. `tradingNow` 가 거짓이라 전종목
 * 스냅샷·화면 갱신이 쉬고 15:30 정규장 종가가 그대로 남는다. 그래야 그 사이 저장되는 종목 등락 수·테마 기록·
 * 테마 다리(15:40)가 정규장 종가 기준으로 깨끗하다. NXT 움직임은 종목 상세 실시간(소켓)으로 본다.
 *
...
export type Session =
  ...
  | "정규장" // 09:00~15:30
  | "공백" // 15:30~16:00 (9/14~) — KRX 연속매매가 쉰다(NXT 애프터는 15:30 단일가·15:40 연속으로 돈다)
...
export function sessionOf(date: string, minute: number): Session {
  ...
  /*
   * 9/13 까지는 15:30 직후부터 NXT 애프터가 이어 받는다. 9/14 부터는 **KRX 가** 30분 쉰다(16:00 애프터부터).
   * NXT 는 이때도 돈다(15:30~15:40 단일가) — 화면 글자는 「거래 없음」이 아니라 「KRX 휴장 · NXT 애프터」다.
   */
...
/**
 * **마감 뒤 정리(파이프라인)를 몇 시에 시작하나** — 위 `cleanupStartMinute` 과 갈랐다 (2026-09-16).
 *
 * 벤티지: "마감 정리를 3시 40분에 시작하는 게 어떨까 싶네. 보니까 3시 40분에도 정규장 집계가 다
 * 나오더라고. 그때 해야 4~5시 사이에 끝나고 **애프터장에서 새로운 신호를 확인하고 거래**할 수 있을
 * 것 같거든. 8시 이후에 해 버리면 애프터장에서의 거래를 놓치게 되니까."
 *
 * 맞는 지적이고, 앞뒤도 그게 맞다: **우리 판정은 이미 전부 정규장 종가 기준**이다(신호등 표본·
 * 주도주 15:35 기록·`regularCloseSettled`). 그런데 파이프라인만 20:10 이라 **정규장 기준 판정을
 * 애프터가 끝난 뒤에 내놓고 있었다** — 쓸 시간이 없는 신호였다.
 *
 * 그래서 둘을 나눈다:
 *
 *   `pipelineStartMinute` 15:55 — **판정**을 만든다. 애프터(16:00~20:00)에서 쓸 수 있게
 *   `cleanupStartMinute`  20:10 — **그날 값이 굳는 시각**. 원장의 「오늘 줄을 담아도 되나」,
 *                                 업종 수급 마감, 저녁 주문 정합성은 그대로 여기를 본다
 *
 * 그리고 20:10 에는 **마무리 회차**가 짧게 돈다 — 일봉(애프터까지 포함한 그날 전부)과 공매도·대차
 * 확정값만 다시 받아 덮는다. 종가 `c` 는 `mergeBars` 가 정규장 값으로 지키므로 판정 기준은 안 흔들린다.
 * ## 왜 15:40 이 아니라 **15:55** 인가
 *
 * 벤티지가 말한 시각은 15:40 이다. 그런데 **15:40 에는 종배 스캔이 이미 돈다**(`closeBetScan`,
 * 15:40~15:51 · 13목록 · 1,565종목). 거기에 일봉 전종목(2,800콜)을 같이 태우면 **둘 다 초당
 * 제한에 걸린다** — 종배는 그날 실제로 사는 판단이라 밀리면 안 된다.
 *
 * 그래서 종배 스캔이 끝난 직후로 둔다. 끝나는 시각은 15분 늦을 뿐이고, 애프터(16:00~20:00)는
 * 그대로 다 쓸 수 있다.
 */
export function pipelineStartMinute(date: string): number {
  return afterMarketEra(date) ? 955 : 940;
}

/** 마무리 회차(일봉·공매도·대차 확정값) — 애프터마켓 시대에만 있다 */
export function pipelineWrapMinute(date: string): number {
  return afterMarketEra(date) ? 1210 : -1;
}
