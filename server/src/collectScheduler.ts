 ...
 *
 * ## 시각 — 15:45 (2026-09-01 실측으로 고침)
 *
 * 처음엔 16:00 으로 두고 「2,444콜이면 9분」이라 계산했다. **틀렸다.** 실제로
 * 재 보니 2,751종목에 **30분**이 걸렸다(16:00 → 16:30). 조회 간격 220ms 만 세고
 * 응답 시간을 안 셌다.
 *
 * 16:30 은 신호등 분석이 시작하는 시각이라 딱 붙어서 끝났다. 종목이 늘거나
 * 응답이 느려지면 그대로 겹친다 — 둘 다 키움 한도를 끝까지 먹는 작업이라
 * 겹치면 서로 느려지고 429 가 난다.
 *
 * 15:45 로 당긴다. 30분이면 16:15 에 끝나 16:30 신호등 분석까지 15분 여유가 있다.
 *
 * ⚠️ 장 마감(15:30) 직후라 종가는 확정됐다. 다만 추적기(15:40)·슈퍼신호등(15:45)과
 * 겹친다 — 그 둘은 종목당 조회라 일봉과 한도를 나눠 쓰게 되지만, 셋 다 조금씩
 * 느려지는 편이 **일봉이 낡은 채로 분석이 도는 것**보다 낫다.
 */
const BARS_HHMM = 15 * 60 + 45;
