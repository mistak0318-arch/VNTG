 ...
 *
 * ## ⚠️ 기본 꺼짐 (2026-08-26 장중 실측으로 뒤집음)
 *
 * 밤 탐침(8초)에서는 2번 소켓 LOGIN·REG 가 수락돼 「가능」으로 봤는데, 장중에
 * 다시 재보니 곧바로 **`R10001: 동일한 App key로 접속… 기존 세션은 종료가
 * 됩니다`** 가 오고 한쪽이 닫힌다 — 같은 앱키로 웹소켓은 **사실상 1개만**
 * 유지된다. 켜 두면 1·2번이 서로를 밀어내는 핑퐁이 되어(오늘 아침 그 사태)
 * 실시간이 통째로 불안정해진다. `REALTIME_DUAL=1` 은 실험용으로만 남긴다.
 */
export function dualEnabled(): boolean {
  return RealtimeClient.enabled && process.env.REALTIME_DUAL?.trim() === "1";
}
 ...
 */
/*
 * ⚠️ 95 였다가 90 으로 (2026-08-26 아침 실측).
 * 95×2=190 + VI + **화면 몫(transient)** 이 겹치니 200 을 넘겨 105118
 * (허용 개수 초과)이 3초마다 반복됐다 — 화면이 미국탭(FE 10)이나 보드를 열면
 * 여유 10 이 바로 바닥난다. 180 + VI + 화면 19 까지 안전.
 */
const PER_MARKET = 90;
/**
 * 2번 연결의 시장당 몫 — 1번이 못 담은 **다음 구간**(91~185위)을 가져간다.
 * 0B 하나만 걸므로 190종목이어도 연결 정원(200) 안이다.
 */
 ...
 *
 * 그래서 스케줄러 몫을 줄여 **화면 몫을 비워 둔다.** 화면이 그 자리를 다 쓰면
 * 오래 본 것부터 빠진다(`subscribeTransient`) — 스케줄러 몫은 안 밀린다.
 *
 * ## ⚠️ 190 → 175 (2026-09-09 실측)
 *
 * 아침 08:07 에 미니PC 를 보니 **`seats: keep 200 / max 200`, `transient: 0`** 이었다.
 * 정원이 통째로 차서 **화면 몫이 0** 이다 — 거래상위에서 종목을 눌러도 실시간이 안 붙는다.
 * 「190 만 쓴다」는 이 주석이 지켜지지 않고 있었다.
 *
 * 스케줄러만 세었기 때문이다. 실제로 `keep` 을 쓰는 곳이 셋이다:
 *   · 스케줄러 국내 종목            190
 *   · VI(`1h`)·장운영구분(`0s`)     005930 하나 (겹쳐서 0)
 *   · **자동감시 종목**(`ensureLiveCode`) 포지션·감시마다 하나씩 — 이게 안 세어졌다
 *
 * 포지션이 늘수록 화면 몫을 먹고, 200 에 닿으면 그때부터 조용히 거절된다. 175 로 낮춰
 * **자동감시 15 + 화면 10** 을 남긴다. 시세를 볼 종목이 15 줄지만, 그건 순위 하위권이고
 * 「눌렀는데 안 뜨는」 쪽이 훨씬 나쁘다.
 */
const MAX_CODES = 175;
