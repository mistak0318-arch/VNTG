...
export const DEFAULT_RULES: CisRules = {
  ...
  maxPerStock: 12,
  /*
   * 여덟 → **열** (2026-09-04, 벤티지: "약간은 적극적인 투자 모드로 변경해줘").
   * 한 종목 12% 는 그대로 둔다 — 자리를 늘리는 것과 한 자리를 키우는 것은 다른
   * 이야기이고, 갭하락 한 방에 계좌가 끝나는 것은 **뒤쪽**이다. 자리만 늘린다.
   */
  maxPositions: 10,
  /*
  ...
  targetPct: 15,
  trailDropPct: 0,
  /* 열흘 → **열이틀** (2026-09-04 적극 모드). 추세에 조금 더 시간을 준다 */
  maxHoldDays: 12,
  /* 신호등 분석 원장을 후보에 더한다 — 조회를 안 늘리면서 보는 자리가 넓어진다 */
  useListTrack: true,
  /* 신조 ① — 경보 있는 종목은 점수와 무관하게 뺀다 */
  rejectAlerts: true,
  /* 60 → **55** (2026-09-04 적극 모드) — 후보의 폭만 넓힌다. 경보·장세 문은 그대로다 */
  minScore: 55,
  /*
   * 얇은 종목은 내가 사는 수량에 값이 밀린다 — 모의라도 못 살 걸 샀다고 적지 않는다.
   *
   * 500 → **300억** (2026-09-04 적극 모드). 벤티지가 예전에 말한 하한이 100억이었고
   * 여기는 그보다 세게 잡아 두었는데, 500 에서는 원장의 초록 상당수가 **신호등을 재기도
   * 전에** 잘렸다(종배 마른 실행에서 확인). 300 이면 "잡주는 안 되"라는 방향은 지키면서
   * 후보가 남는다. 100 까지는 안 내린다 — 그건 방향이 뒤집히는 자리다.
   */
  minTradeValue: 300,
  /* 시총 1000 → **700억** (2026-09-04). 하루 소나기 잡주를 막는 바닥은 그대로 있다 */
  minMarketCap: 700,
  /* 40 → **35** (2026-09-04 적극 모드) — 시장 문을 한 칸만 연다. 장세 신뢰도 문은 안 건드린다 */
  minMarketScore: 35,
  /*
  ...
  useRegimeGate: true,
  /*
   * +7 → **+6%** 에서 손절을 본전으로 (2026-09-04).
   *
   * 이건 적극 모드의 **대가**다. 위에서 자리를 늘리고 문턱을 낮췄으니 들어오는 것이
   * 늘고, 그중 틀린 것도 같이 는다. 본전 전환을 한 칸 앞당겨 **이익을 손실로 바꾸는
   * 날**을 줄인다. 적극적으로 들어가되 지키는 것은 더 빨리 지킨다.
   */
  trailAfterPct: 6,
  ...
  useClose: true,
  /*
   * 갭 +4 → **+5%** 까지 (2026-09-04 적극 모드). 손절이 -7% 이므로 5% 를 주고 들어가면
   * 남은 거리가 2% 다 — 여기가 한계선이고 더 열지 않는다. 6% 부터는 사는 순간 진 자리다.
   */
  maxOpenGap: 5,
