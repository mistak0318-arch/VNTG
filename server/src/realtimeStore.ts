...
/**
 * **장 기준 날짜.** 자정이 아니라 **아침 07:50 에** 넘어간다.
 *
 * 자정으로 끊으면 새벽 2시에 복기하려고 열었을 때 **방금 끝난 장이 사라진다** —
 * 날짜만 바뀌었을 뿐 다음 장은 아직 시작도 안 했는데. HTS·MTS 가 전 장 데이터를
 * 다음 날 아침까지 들고 있는 게 그래서다.
 *
 * ## 왜 07:50 인가 (2026-09-09, 벤티지)
 *
 * 처음엔 새벽 6시였다. 그런데 벤티지: "다음 날 오전 7시 50분까지는 전날 장 마감 시세(KRX·NXT
 * 마감)를 잡고 있는다. 그래서 복기를 다음 날 아침까지 가능하도록 만든다."
 *
 * 맞는 자리다. **07:50 은 우리 스케줄러가 실시간에 붙는 바로 그 시각**이고
 * (`realtimeHub.shouldRun`), 새 장의 첫 체결은 NXT 프리마켓이 열리는 **08:00** 부터다.
 * 그 사이 10분이 통째로 여유라, 경계를 여기 두면
 *
 *   · 전날 장이 **아침까지 「오늘」로 남는다** — 복기 화면이 「지난 장」으로 라벨을 바꾸지 않는다
 *   · 새 날의 첫 프레임이 옛 날짜 파일에 섞일 위험이 없다 (첫 틱이 10분 뒤다)
 *
 * 6시로 끊었을 때는 06:00~07:50 사이에 열면 방금 끝난 장이 이미 「지난 장」이었다.
 * 그 두 시간이 정확히 **아침에 복기하는 시간대**다.
 */
/** 하루가 넘어가는 시각 (KST 분). 스케줄러가 붙는 07:50 과 같은 자리 */
const SESSION_ROLL_MIN = 7 * 60 + 50;

function sessionDay(now = new Date()): string {
  /* +9시간(한국) − 07:50(장 기준) = +1시간 10분 뒤 UTC 자정이 경계가 된다 */
  const shiftMin = 9 * 60 - SESSION_ROLL_MIN;
  const k = new Date(now.getTime() + shiftMin * 60 * 1000);
