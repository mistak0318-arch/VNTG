...
/* 살아 있다는 신호 · 08:55 아침 점검 · 저녁 정합성(애프터 마감 뒤 20:10) (개편 ②) */
...
/* 장세 점검 (2026-08-31) — 장중 재고. 표본은 마감 뒤 정리가 매일 만들고, 그게 실패한 날만 21:30 그물 */
startRegimeScheduler(client);
/* CIS 일지 — 설정에서 켜야 실제로 돈다(기본 꺼짐) */
startCisScheduler(client);
/* 종배 스캔 (2026-09-15) — 15:40 에 오늘 초록만 따로. 파이프라인(15:55)보다 앞이라 종배 판단이 안 밀린다 */
...
/* KRX(KIND) 공시 — 07~22시 10분마다 오늘치 (2026-09-10) */
startKrxNoticeCollector(client);
...
/* 체결 창고 — 차트 「복기」가 읽는다. 지난 90일 채우고 장중 30분마다 오늘치 (2026-09-10) */
startFillStore(client);
/* 데이터 보관 기간 정리 (2026-08-31) — 실시간 로그가 무한히 쌓고 있었다 */
startRetentionScheduler();
/* 「이 종목을 담은 ETF」 역인덱스 — 하루 1회(애프터 마감 20:00 뒤). 화면은 파일만 읽는다 */
...
startThemeScheduler();
/* 전종목 일봉 캐시 — 하루 1회(애프터 마감 20:00 뒤). 테마 5·20일 누적과 신호등이 같이 쓴다. 파이프라인도 제 차례에 부른다 */
...
/*
 * **전종목 일별 수집** (2026-09-01) — 마감 뒤 정리(15:55)가 제 차례에 부른다. 공매도·대차만 20:10 마무리에서.
 *
 ...
 *
 *   일봉 → 원장 → 장세 → 추적기 → **신호등 분석 → 슈퍼신호등** → 교차 → 전종목 마크
 *   → 수출입 → 실적 캐시 → 표본,  그리고 20:10 마무리(일봉·공매도·대차 확정값)
 *
 * ⚠️ 분석이 슈퍼신호등보다 **먼저**다 — 슈퍼는 분석이 받아 둔 목록으로 교집합을 낸다.
 *   (여기 순서를 거꾸로 적어 둬서 한동안 코드와 문서가 서로 다른 말을 했다)
 *
