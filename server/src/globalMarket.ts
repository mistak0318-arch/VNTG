...
const TARGETS: {
  ...
  /*
   * 엔화 둘 (2026-09-04, 벤티지 요청). 야후 티커는 실측으로 확인했다 —
   * JPY=X 155.926 (달러당 엔) · JPYKRW=X 8.673 (엔당 원).
   *
   * **왜 둘인가.** 성격이 다르다.
   *   달러/엔  엔 캐리의 온도계다. 엔이 강해지면(숫자가 내리면) 빌린 엔을 갚느라 전 세계
   *            위험자산에서 돈이 빠진다 — 금리 카드의 「일본 10년」 설명과 같은 줄기다.
   *   엔/원    실제로 우리가 쓰는 환율. 여행·수입 말고도 자동차·기계처럼 일본과 같은 것을
   *            파는 업종이 여기에 붙는다.
   *
   * ⚠️ 엔/원은 **엔 하나당 원**이라 8.67 로 온다. 한국에서 흔히 쓰는 「100엔당 867원」이
   * 아니다. 값을 100배 해서 보여 주고 싶었지만 그러면 **눌러서 여는 야후 차트와 숫자가
   * 갈린다** — 카드가 867, 차트가 8.67 이면 어느 쪽이 맞는지 사람이 판정해야 한다.
   * 그래서 야후가 준 값 그대로 두고, 이름에 기준을 적는다.
   */
  { key: "usdjpy", label: "달러/엔", group: "환율", symbol: "JPY=X" },
  { key: "jpykrw", label: "원/엔 (1엔)", group: "환율", symbol: "JPYKRW=X" },
