 ...
 * ## 예산 — 초당 1콜만 쓴다
 *
 * 제한의 20%다. 나머지는 신호등·시세분석·알림 몫으로 남긴다. 종목당 2콜(거래원+프로그램)이라
 * **2초에 한 종목** — 대상이 80종목이면 **한 바퀴가 160초**다. 거래원 누적은 그 정도 간격이면
 * 흐름을 잃지 않는다(화면이 열려 있을 때 쓰는 주기가 30초다). 아래 `STEP_MS` 주석 참조.
 *
 * ## 무엇을 고르나
 *
...
/** 한 바퀴에 쓸 상한 — 초당 1콜 × 160초 */
const MAX_CODES = 80;

/**
 * 종목 하나 사이의 간격(ms).
 *
 * ⚠️ 1000 → 2000 (2026-09-23 — 벤티지: "미니PC 에 바로 붙었는데 종목 상세 들어갈때도 로딩이 심하고 시세분석에
 * 각탭 옮길때도 왜케 로딩이 심하냐"). 위 설계는 「초당 1콜 = 예산의 20%」인데, 09-11 에 `brokerFlow` 가
 * 프로그램(`ka90013`)까지 같이 부르게 되면서 **종목당 2콜**이 됐다 — 초당 2콜, 4.5/s 버킷의 44%.
 * health.json 실측(09-23 10:06): 개장 66분에 `ka10040` 3,908 · `ka90013` 3,950, 대기열 평균 146ms ·
 * 최대 223ms. 종목 상세는 TR 을 여럿 부르니 콜마다 그만큼 기다려 몇 초가 됐다.
 * 2초에 하나면 다시 초당 1콜이다. 한 바퀴 160초 — 거래원 누적은 그 간격이면 흐름을 잃지 않는다.
 */
const STEP_MS = 2000;
 ...
 * KRX 애프터마켓의 창구 이동까지). 15:30~16:00 은 KRX 가 쉬어(NXT 는 돈다) KRX 거래원이 안 바뀌므로 뺀다.
 */
