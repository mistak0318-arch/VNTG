...
export async function themeStrength(
  ...
  if (market === "kr") {
    for (const t of store.themes) {
       ...
       */
      /*
       * ⚠️ **여기서 덮어쓰는 값들은 「직전 거래일까지」다** (2026-08-31 점검).
       *
       * 위 `build()` 는 오늘 평균을 뒤에 붙여 연속일수를 셌지만(`withToday`), 바로
       * 아래에서 **일봉 캐시 기준 값이 그것을 덮는다.** 캐시는 하루 한 번(16시)
       * 만들어지므로 장중에는 오늘이 아직 안 들어 있다.
       *
       * 그래서 「오늘 +0.52% 인데 연속 0」 같은 줄이 나온다 — 등락률은 실시간이고
       * 연속·누적은 어제까지라 **서로 다른 날을 보고 있다.**
       *
       * 캐시 쪽을 쓰는 것은 맞다(실제 일봉이라 기록 평균보다 정확하다). 대신 화면에
       * 그 기준을 적는다 — 나란히 놓인 숫자가 같은 날 것처럼 읽히면 안 된다.
       */
