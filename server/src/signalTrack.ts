...
export const DEFAULT_TRACK_CONFIG: TrackConfig = {
  tiers: [70, 80, 90],
  /*
   * 60 → 300 (2026-08-25, 사용자 요청 — 같은 날 200 을 거쳐 300 으로).
   * 상위 60 은 다 아는 종목이라 검증 표본이 얇았다. 하루 한 번(15:40) 도는
...
async function load(): Promise<Store> {
  try {
    ...
    /*
     * 옛 기본값 이사 (2026-08-25). 저장분의 60·200 은 사람이 고른 값이 아니라
     * **옛 기본값이 저장을 타며 박제된 것**이다 — 새 기본 300 으로 올린다.
     * (200 은 같은 날 잠깐 기본이었다) 그 밖의 값은 사람이 정한 것이라 안 건드린다.
     */
    if (config.universe === 60 || config.universe === 200)
      config.universe = DEFAULT_TRACK_CONFIG.universe;
    return {
