...
export const DEFAULT_CONFIG: SignalConfig = {
  ...
  checks: [
    // ---------------- 추세 ----------------
    {
       ...
       */
      /*
       * **강세장 전용으로 켠다 · 무게 2** (2026-09-02 저녁, 세대 4 — 벤티지: "정배열 이런것도
       * 적용이 안되는걸까").
       *
       * 혼자서는 힘이 0 이다(시총 중립 완전 정배열 -0.0/49, 약세장 -2.9/42). 그런데 세대 3
       * 기본값 위에 얹어 실전 채점기로 재니 **강세장에서만** 값을 한다:
       *   무관 w1   초록 3,158 → 2,194 · +3.6/58 · 약세 +2.3     ← 깎인다
       *   강세 w2   초록 3,158 → **3,443 · +4.6/61** · 강세 +4.0 → +4.5 · 블록 넷 다 양수
      ...
      threshold: 0,
      strongAt: 0,
      hint: "현재가 ≥ 짧은 이평선 ≥ 긴 이평선 (아래에서 선 선택). 완전 정배열 100, 가장 짧은 선 위 50. **강세장에서만** — 약세장에선 정배열이 -2.9%p 였다",
      cost: 0,
    },
    {
      key: "nearHigh",
