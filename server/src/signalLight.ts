 ...
 *
 * 벤티지 요청("점수점검 하고 백테스팅까지해봐" → "모든 경우의수 모든 조합")으로
 * 거래대금 상위 500 종목 × 400 거래일 = **19만 관측**을 놓고 켤 수 있는 기준의
 * **모든 조합(248개)**을 돌렸다. 표본을 날짜로 반 갈라 앞에서 고르고 **뒤에서
 * 채점**했다 — 전 구간 평균만 보면 반드시 과최적화되기 때문이다.
 *
 * 실제로 그 분할이 사고를 막았다. 120일 표본에서 「+7.95%, 압도적」이던 조합이
 * **뒤쪽 절반에서 -19%p** 였다. 앞쪽 상승장이 그 숫자를 전부 만들고 있었다.
 *
 * **바꾼 것 — 방향이 뚜렷하고 두 표본에서 일관된 것만:**
 *
 *   테마 강세    끔   가르는 힘 -5.76%p (거꾸로 작동)
 *   20일선 이격도 끔   가르는 힘 +2.45%p 인데 위험 축이라 초록을 막고 있었다
 *   (고점 근접·거래대금은 이미 꺼져 있었다 — 각 항목 주석 참고)
 *
 * **결과 (정배열 · 60일 신고가 · 위쪽 매물 부담 · 5일선 이격):**
 *
 *   초록 27,380건  20일 +5.70% (승률 52%)   시장 +4.19% (52%)  → +1.51%p
 *   앞쪽 +0.51%p · 뒤쪽 +2.92%p — **양쪽 다 양수**
 *   구간별 단조 증가: 0~44 +3.72 → 45~59 +4.89 → 70~79 +5.07 → 90~100 +6.21
 *
 * 마지막 줄이 핵심이다. 예전 설정에서는 점수 구간이 톱니였다(80~89 가 70~79 보다
 * 나쁨). **점수가 높을수록 좋아야** 점수라고 부를 수 있다.
 *
 * ⚠️ **훑기 1등을 그대로 쓰지 않았다.** 1등은 「60일 신고가 + 5일선 이격」
 * (+1.97%p)이었지만, 그러면 사실상 신고가 여부만 보는 이진 판정이 되어 점수
 * 체계가 무의미해진다. 248개 중 최고를 고르는 것 자체가 과최적화다.
 *
 * ⚠️ **여섯 기준은 아직 채점 밖이다** — ETF 뒷배·수급 3종·영업이익·시가총액.
 * 그때의 값을 우리가 안 갖고 있어 되짚을 수 없을 뿐, **틀렸다는 근거는 없다.**
 * 잴 수 없는 것을 껐다가는 재현 가능한 것만 남는 신호등이 된다.
 *
