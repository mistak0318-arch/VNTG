...
export interface SuperConfig {
   ...
   * ## 300 → 500 (2026-08-31)
   *
   * 「신호등 찾기에서 500 으로 놓고 찾으면 나오는 종목이 슈퍼에는 영영 안 들어온다」는
   * 문제를 없앤다. 두 화면의 모집단이 같아야 「여기선 보이는데 저기선 안 보인다」가
   * 안 생긴다.
   *
   * ⚠️ **평가 상한과 같이 봐야 한다.** 모집단을 키우면 교집합 통과분이 늘어나는데
   * 상한(`maxEval`)이 그대로면 **늘어난 만큼 그대로 잘린다** — 잘린 종목은 초록이었을
   * 수도 있는데 재보지도 못한다. 실행 뒤 화면이 「N종목이 잘렸다」를 적으니 그걸 보고
   * 상한을 같이 올려야 한다.
   */
  universeSize: number;
  /**
...
export const DEFAULT_SUPER_CONFIG: SuperConfig = {
  ...
  rainbowDays: 2,
  /* 신호등 찾기와 같은 모집단으로 (2026-08-31) — 위 주석 참고 */
  universeSize: 500,
  maxEval: 40,
  weakRegimeMode: "mark",
};
