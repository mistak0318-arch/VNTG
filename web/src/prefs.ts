...
const LOCAL_ONLY = [
  ...
  "vntg.lock.locked",
  /*
   * ## 전수 검사로 찾은 넷 (2026-09-01)
   *
   * 화면 잠금을 고친 뒤 **같은 병을 앓는 것이 더 있는지** 훑었다. 판별 기준은
   * 하나였다 — **주석이나 화면이 「이 기기」라고 말하는데 `LOCAL_ONLY` 에 없는 키.**
   *
   * 넷이 더 나왔다. 전부 화면 크기에 딸린 값이라 전역이면 27인치 설정이 폰까지
   * 따라온다.
   */
  /** 카드 접힘 — "접힌 상태를 기기별로 기억해서". 폰에서는 접어 두고 싶다 */
  "vntg.settings.open",
  /** 차트 설정 — "기기마다 화면 크기가 달라" */
  "vntg.chart",
  /** 투자자 표에서 고른 열 — "고른 것은 이 기기에 남고". 열 개수는 화면 폭에 딸린다 */
  "vntg.investor.cols",
  /** 미국 보드에서 보는 그룹 — "모니터마다 보는 자리가 다르다" */
  "vntg.usboard.group",
