...
async function req(path: string, init?: RequestInit): Promise<Response> {
  try {
    ...
    /*
     * JSON 자리에 HTML 이 오는 경우(Access 로그인 페이지·서버 기본 404)는 **`installApiGuard` 가 입구에서**
     * 잡는다(`authGuard.ts`). 여기 같은 검사를 두면 절대 닿지 않는 죽은 코드라 뺐다 — 2026-09-21 회귀 점검.
     * 컴포넌트 여럿이 `fetch` 를 직접 부르므로 입구 한 곳이 맞는 자리다.
     */
    return res;
  } catch (e) {
    noteFetchFailure();
...
export const api = {
  ...
  /** 주문 비밀번호를 옛 것 없이 새로 — 앱 아이디·비밀번호로 (2026-09-08) */
  orderPasswordReset: (username: string, password: string, next: string, kind: "text" | "pattern") =>
    orderPost<{ ok: boolean }>("/api/order/password/reset", { username, password, next, kind }),
...
export interface ScreenHit {
   ...
   * 실측 29,570관측: 초록 +1.8%p/55% → **초록 ∩ 외인3 = +3.2%p/59%** (하루 32.8 → 17.3개).
   */
  fgn3?: boolean;
  twin?: boolean;
...
export interface RankResult {
  ...
  rows: (Record<string, unknown> & {
    ...
    etf: boolean;
    /**
     * **원장에서 더한 수급(억원)** — 5·10·20거래일 합을 한꺼번에 (2026-09-09). 순위 TR 은
     * 안 주는 값이라 서버가 원장에서 얹는다. 원장이 없는 종목은 값이 null, `days` 는
     * 실제로 더한 날 수(원장이 얕으면 기간보다 적다).
     */
    flow?: Record<string, FlowSum>;
...
/** 한 종목의 N거래일 순매수 합(억원) — 시세분석 줄마다 붙는다 */
