...
const STEPS: { key: string; label: string; why: string; heavy?: string }[] = [
  ...
  },
  {
    key: "finance",
    label: "⑩ 실적 캐시 (주 1회)",
    why: "표본의 실적 칸을 채울 재료 — 7일 지난 것만 다시 받습니다. 평소엔 몇 초, 분기마다 15~20분",
    heavy: "예산 25분",
  },
  {
    key: "samples",
  ...
  /*
   * 마무리 회차 (2026-09-16) — 정규 회차에는 안 돈다. 화면에는 보여야 한다:
   * 20:10 에 이 둘이 성공했는지가 「그날 값이 최종인가」다.
   */
  {
    key: "barsFinal",
    label: "⑫ 일봉 마무리 (20:10)",
    why: "애프터까지 포함한 그날 시·고·저·거래량. 종가는 정규장 값으로 지킵니다 — 신호등 기준은 안 흔들립니다",
    heavy: "약 30~40분",
  },
  {
    key: "ledgerFinal",
  ...
  return (
    <div className="ac">
      <p className="table-note">
        평일 <b>15:55</b>에 자동으로 <b>차례대로</b> 돕니다. 앞의 것이 뒤의 것의 바탕이라
        순서가 중요합니다 — 예전에는 시각으로만 잡혀 있어서 슈퍼신호등이 일봉보다 먼저
        돌고, 신호등 분석이 원장보다 먼저 도는 일이 있었습니다.
      </p>
      <p className="table-note">
        <b>왜 15:55 인가</b> — 우리 판정은 전부 <b>정규장 종가 기준</b>입니다. 그래서 정규장이 끝나면
        바로 만들어 <b>애프터장(16:00~20:00)에서 쓸 수 있게</b> 합니다. 9/14~9/15 에는 20:10 이었는데,
        그러면 판정이 나올 때 이미 거래할 시장이 없었습니다. 그날의 <b>최종값</b>(애프터까지 포함한
        일봉 · 저녁에 공표되는 공매도·대차)은 <b>20:10 마무리 회차</b>가 따로 받아 덮습니다.
        15:40 이 아니라 15:55 인 것은 <b>종배 스캔</b>(15:40~15:51)과 키움 조회가 겹치지 않게 하려는 것입니다.
      </p>
