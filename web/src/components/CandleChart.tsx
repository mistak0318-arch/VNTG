...
export function CandleChart({
  ...
  useEffect(() => {
    ...
    /*
     * **봉이 바뀌면 띠도 다시 그린다** (2026-09-23 — 벤티지: "얘도 이상해!!" → "봉을 클릭하면
     * 잘 나오네").
     *
     * 리사이저는 `if (narrow && !bar.innerHTML)` 일 때만 띠를 채운다 — 십자선이 읽고 있는 값을
     * 덮지 않으려던 조건인데, **탭을 옮겨 봉이 통째로 바뀌어도 안 채우게** 만들고 있었다.
     * 그래서 1분봉에서 보던 「09/23 01:32 · 193 193 193 193」이 일봉으로 옮긴 뒤에도 그대로
     * 남았다. 봉을 누르면 `onMove` 가 새로 그리니 그때만 맞았던 것이다 — 벤티지가 짚은 그대로다.
     *
     * 여기는 **데이터가 실제로 바뀐 자리**라 덮어도 안전하다. 십자선이 올라가 있으면 다음
     * 움직임에 곧바로 제 값으로 다시 그려진다.
     */
    if (narrowRef.current) fillBarRef.current?.(candles.length - 1, intraday);
