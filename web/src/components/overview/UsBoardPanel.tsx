              ...
              {yieldRows.map((r) => (
                <button
                  ...
                  onClick={() =>
                    setChart({
                      /*
                       * ⚠️ **금리는 야후 경로다** (2026-09-04 고침).
                       *
                       * 여기가 `kind: "usStock"`(한투 해외종목)이었다. 그래서 요약 탭에서 같은 금리를
                       * 누를 때는 차트가 자세히 나오는데 미국 탭에서는 거의 비어 있었다 —
                       * 벤티지: "미국 탭에서 10년물 금리 등 선택하면 차트에 정보가 많이 없네."
                       *
                       * 한투는 ^TNX·^TYX 를 아예 거절한다(`server/src/usMajor.ts` 실측 주석:
                       * "안 되는 것 NDX·RUT·TNX·TYX·환율 — 0 을 주거나 거절한다"). 이 값들은
                       * 애초에 야후에서 받아 카드에 그리고 있었으므로, 차트도 같은 곳에서 받아야 한다.
                       * `kind` 를 안 주면 야후다 — 요약 탭이 쓰던 길과 같아진다.
                       */
                      symbol: r.symbol,
                      label: r.label,
                      digits: r.digits,
