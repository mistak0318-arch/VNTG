...
import { api, signClass, type ThemeBridge, type ThemeStrength } from "../api";
    ...
    <>
      ...
      */}
      ...
      <div className="table-note">
        분류는 <b>네이버</b>에서 주 1회 받아 둔 것이고, 숫자는 우리가 계산합니다 —
        국내·ETF·미국이 같은 자로 재집니다.
        <b>상승비율</b>은 테마 안에서 오른 종목의 비율입니다(몇몇이 끄는지 다 같이 가는지).
        {/*
          기준일이 다르다는 것을 밝힌다 (2026-08-31 점검). 등락률·상승비율은 실시간
          스냅샷이고, 연속·5일·20일·60일은 하루 한 번(16시) 만드는 일봉 캐시라
          장중에는 오늘이 안 들어 있다. 나란히 놓인 숫자가 같은 날 것처럼 읽히면
          「오늘 +0.5% 인데 연속 0」 같은 줄을 고장으로 읽게 된다.
        */}
        <b>오늘·상승비율</b>은 지금 시세이고, <b>연속·5일·20일·60일</b>은 하루 한 번
        만드는 일봉 기준이라 <b>직전 거래일까지</b>입니다 — 그래서 오늘 오르고 있어도
        연속이 아직 0일 수 있습니다.
      </div>
    </>
  );
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        {t.key.startsWith("kr:") && <UsBridge no={Number(t.key.slice(3))} onSelectStock={onSelectStock} />}
 ...
 */
function UsBridge({ no, onSelectStock }: { no: number; onSelectStock: (code: string, name: string) => void }) {
