...
function downloadCsvTemplate() {
  const lines = [
    ...
    "# 종류 (선택) 주간 · 증시 · 공개 · 회의 · 실적 · 채권 · 파생 · 휴장 · 이벤트 · 학회 · 개인",
    "#            (비우면 개인)  ※ 증권사 캘린더의 분류를 그대로 쓸 수 있습니다",
    "#            공개=지표 발표 · 회의=중앙은행/정상회담 · 채권=국채 입찰 · 파생=선물옵션 만기",
    "#            주간=그 주의 핵심 요약. 그 주 **일요일** 날짜로 넣으면 주간 화면 맨 위에 뜹니다",
    ...
    "#            주간 핵심은 여기에 설명 문단을 적습니다",
    "# 국가 (선택) 한국 · 미국 · 중국 · 일본 · 유로존 · 영국 · 독일 · 대만 · 인도 · 글로벌",
    "# 대표 (선택) 그날의 대표 일정이면 「대표」 또는 ★. 주간 화면 맨 위로 올라갑니다",
    "#",
    "# ⚠️ 빈 칸도 쉼표로 자리를 지켜야 합니다 — 2026-09-04,고용보고서,공개,21:30,,미국,",
    "# ⚠️ 같은 파일 이름으로 다시 올리면 그 파일로 넣었던 일정을 지우고 새로 넣습니다.",
    "#    직접 입력한 일정은 건드리지 않습니다.",
    "# ─────────────────────────────────────────────",
    "날짜,제목,종류,시간,메모,국가,대표",
    "2026-08-30,이번 주 핵심,주간,,\"반도체 투심과 한국 수출. 7월 수출이 전년비 +50.5% 늘며 재평가 기대를 높임\",,",
    "2026-09-01,한국 8월 수출입,공개,09:00,전년 대비 증감률,한국,대표",
    "2026-09-04,미국 8월 고용보고서,공개,21:30,실업률·비농업 고용,미국,대표",
    "2026-09-17,FOMC 결과,회의,03:00,\"금리 결정, 점도표\",미국,대표",
    "2026-09-10,미국 10년 국채 입찰,채권,,,미국,",
    "2026-09-11,선물·옵션 동시만기,파생,,네 마녀의 날,한국,",
    "2026-09-30,삼성전자 3분기 잠정실적,실적,,,한국,",
    "2026-10-03,개천절 휴장,휴장,,,한국,",
    "2026-10-14,반도체 학회 발표,학회,14:00~16:00,차세대 HBM 세션,글로벌,",
    "2026-11-05,신제품 언팩,이벤트,23:00~00:00,,미국,",
  ];
...
export function CalendarPage() {
  ...
  /**
   * 보고 있는 주의 7일치 — 주간 브리핑이 쓴다.
   *
   * 날짜순으로 정렬해서 넘긴다. 브리핑은 「이번 주 대표 일정」을 시간 순으로
   * 늘어놓는 것이 뜻이 맞다 — 무슨 요일에 오는지가 곧 준비할 순서다.
   */
      ...
      */}
      ...
      <div className="cal-layout">
        <div className="cal-main">
          ...
          */}
          {/*
            이번 주 브리핑 — **요일별보다 먼저** (2026-08-30, 증권사 리서치 캘린더 양식).

            「이번 주에 뭘 봐야 하나」를 먼저 답하고 나서 날짜로 흩는다. 예전엔 그
            첫 답이 없어서, 주간 화면을 열면 일정 스물몇 개가 요일에 흩뿌려진 것만
            보였다 — 무엇이 중요한지는 사람이 매번 다시 골라야 했다.
          */}
          {view === "week" && (
            <WeekBrief start={weekStart(selected)} events={weekEvents} onOpen={setDetail} />
          )}
