...
export const SCREENER_TABS = [
  ...
  /*
   * 정규장 대비 장외 (2026-09-14). 처음엔 KRX/NXT 괴리율로 붙였는데 벤티지: "이제 괴리율 계산을
   * KRX 나 NXT 기준에서 정규장·애프터장으로 해야 되지 않니? 지금 저건 의미가 없어 보이는데".
   * 맞다 — 애프터마켓이 생겨 16~20시엔 두 거래소가 다 살아 있어 차이가 0 에 붙는다. 기준을 **정규장
   * 종가**로 바꿨다. 아침은 NXT 프리가 앞 장 종가 대비, 저녁은 애프터가 오늘 종가 대비.
   */
  { key: "after-gap", label: "정규장 대비 장외", kind: "rank" as const },
        ...
        <div className="filter-row ctl-ribbon">
          ...
          */}
          ...
          <button
            ...
            title={
              candleOn && !hasCandle && all.length > 0 && !loading
                ? "봉을 켰지만 이번 응답엔 봉이 없었습니다 — 서버가 1.2초 안에 못 받으면 봉 없이 먼저 보냅니다. 다음 새로고침에 붙습니다. 계속 이러면 키움 시세 조회가 막힌 것"
                : "등락률 옆에 당일 봉(시·고·저·현재가). 켤 때만 시세를 더 받습니다 — 100줄이면 조회 두 번"
            }
          >
            {/* 켰는데 칸이 안 보이면 이유를 단추가 말한다 — 전엔 칸만 조용히 사라졌다 (2026-09-23 전수검토 (나)) */}
            📊 봉 {candleOn ? (!hasCandle && all.length > 0 && !loading ? "끄기 · 아직" : "끄기") : "켜기"}
          </button>
          {/*
                    ...
                    >
                      ...
                      <td className="sticky-col" title={r.name}>
                        ...
                        */}
                        ...
                        {/*
                          조회순위 — **새로 진입**. 직전 응답에 없던 종목이다. 순위 변동(±N)은
                          이미 있던 종목의 얘기라, 눈이 새로 몰린 곳은 이것으로 본다.
                        */}
                        {r.enteredAt && (
                          <i
                            className="scr-new"
                            title={`${Math.max(0, Math.round((Date.now() - new Date(r.enteredAt).getTime()) / 60_000))}분 전 목록에 들어옴`}
                          >
                            NEW
                          </i>
                        )}
