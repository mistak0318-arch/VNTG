      ...
      {job && (
        <>
          ...
          */}
          ...
          {/*
            합의 보드 설명 (2026-08-28 사용자 요청) — 이 표를 읽는 법.
            접어 둔다: 처음 몇 번만 읽으면 되는 글이 매번 자리를 먹으면 안 된다.
          */}
          <details className="cal-fold">
            <summary>💡 이 표는 「합의 보드」입니다 — 읽는 법</summary>
            <div className="page-note">
              <p>
                주식은 확률 게임이고, 확률은 <b>서로 독립적인 근거가 몇 개나 같은 방향을
                가리키는가</b>로 올라갑니다. 이 표의 각 칸이 그 근거(렌즈) 하나씩입니다:
              </p>
              <p>
                <b>🚦 점수</b> — 신호등의 판정. 추세·수급·실적을 가중평균한 <b>입장권</b>입니다.
                여기 나온 종목은 이미 이 문턱을 넘었습니다.
                <br />
                <b>테마</b> — 이 종목이 든 <b>무리</b>가 오늘 도는가. 혼자 오르는 종목과 무리와
                함께 오르는 종목은 다음 날이 다릅니다. <i>N일↑</i>은 연속성 — 하루 반짝과
                사흘째는 다른 판입니다.
                <br />
                <b>ETF 뒷배</b> — 그 무리에 <b>실제로 돈을 태우는 상품</b>이 가는가. 테마가
                말이라면 ETF 는 돈입니다.
                <br />
                <b>🌟</b> — 슈퍼신호등: 일곱 목록의 <b>교집합</b>에도 걸려 추적 중인 종목.
                가장 좁은 문을 통과한 표시일 뿐, 이 표의 범위를 좁히지는 않습니다.
              </p>
              <p>
                읽는 법: <b>점수가 비슷하면 렌즈가 더 많이 동의하는 쪽</b>이 확률이 높은
                자리입니다 — 초록 + 테마 상승 + 뒷배 상승 + 🌟 가 겹치면 넷이 따로 잰
                값이 같은 말을 하는 것입니다.
              </p>
              <p>
                ⚠️ 단, 렌즈는 판을 탑니다. 자체 백테스트(최근 55거래일)에서 <b>과열
                구간에는 강한 테마 소속이 오히려 위험을 키웠습니다</b> — 강한 무리는 꼭대기도
                같이 가파릅니다. 시장 신호등(맨 위)이 식어 있을 때는 렌즈 동의를 매수
                근거가 아니라 <b>변동성 경고</b>로 읽으세요. 렌즈별 효과 검증은 옆의
                「🧪 신호등 백테스트」에서 직접 돌려볼 수 있습니다.
              </p>
            </div>
          </details>
